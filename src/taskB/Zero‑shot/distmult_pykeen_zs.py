...
class DistMultModel(nn.Module):
    ...
    def forward(self, h: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        """
        # DistMult scoring function: <h, t>
        scores = torch.mm(h, t.t())  # [n_terms x n_types]
        return scores

class PykeenZeroShotDistMultWithFFN:
    def __init__(self, model_name: str = "Qwen/Qwen3-Embedding-4B", device: str = "cuda"):
        ...
        self.distmult = DistMultModel(embedding_dim=self.embedding_dim).to(device)
        
        # Dynamic threshold parameters
        self.min_types = 1  # Minimum number of types to assign
        self.max_types = 3  # Maximum number of types to assign
        self.z_score_threshold = 1.0  # Initial z-score threshold
    ...
    def _compute_scores(self, h: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        """
        Compute DistMult scores for all term-type pairs
        h: term embeddings [n_terms x dim]
        t: type embeddings [n_types x dim]
    ...
    def predict(self, terms_data: List[Dict[str, Any]], types: List[str], output_file: str) -> None:
        ...
        scores = self._compute_scores(
            term_embeddings, type_embeddings
        ).cpu().numpy()
...
def run_pipeline(data_dir: str, output_dir: str):
    ...
    # Initialize model
    model = PykeenZeroShotDistMultWithFFN()
    
    # Run prediction
    print("\nRunning Qwen DistMult...")
    model.predict(
        terms_data=terms_data,
        types=types,
        output_file=str(Path(output_dir) / "predictions_zs_distmult_qwen.json")
    ) 